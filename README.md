...
Shell permission prompts offer **Deny**, **Allow once**, and **Allow for this repository**. Repository approvals persist locally across restarts and apply to projects of that repository. Choose **This exact invocation** or **Command prefix — any arguments**. For example, enter `grep` to approve future simple grep invocations, or `npm test` to approve only that subcommand. Prefix rules match full command words, apply to the same shell tool and execution options, and ignore timeout and display description. Compound shell syntax (pipes, chaining, redirects, substitutions or ambiguous quoting) keeps its prompt; the conservative matcher may also prompt for arguments containing shell syntax characters. Exact approvals retain their original matching behavior. Review and revoke approvals under **Repositories → repository settings → Saved command permissions**. Revocation affects future calls; workflow lifecycle restrictions still apply.
